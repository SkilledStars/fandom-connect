...
func TestRuntimeConfigRequiresTheSamePublicOriginFandomAccepts(t *testing.T) {
	for _, rawURL := range []string{
		"https://localhost",
		"https://127.0.0.1",
		"https://gateway.internal",
		"https://gateway.customer.example.com:8443",
	} {
		t.Run(rawURL, func(t *testing.T) {
			setValidRuntimeEnvironment(t)
			t.Setenv("GATEWAY_BASE_URL", rawURL)
			if _, err := LoadRuntimeConfig(); err == nil {
				t.Fatalf("incompatible gateway URL was accepted: %s", rawURL)
			}
		})
	}
}
...
func TestRuntimeConfigValidatesGoogleAdsAPIVersion(t *testing.T) {
	setValidRuntimeEnvironment(t)
	t.Setenv("GATEWAY_GOOGLE_ADS_API_VERSION", "../../v23")
	if _, err := LoadRuntimeConfig(); err == nil {
